...
# ─────────────────────────────────────────────────────────────────────────────
# CUSTOM CANVAS — adds header/footer on every page
# ─────────────────────────────────────────────────────────────────────────────
class ReportCanvas(_canvas.Canvas):
    ...
    def _draw_decorations(self):
        ...
        if page_num > 1:
            self.setFont("Helvetica", 8)
            self.setFillColor(C_MUTED)
            self.drawString(MARGIN, 8, "Generated by AutoReport · CODTECH Task 2")
            self.setFillColor(C_BLUE)
...
def _cover_page(story, styles, analysis):
    ...
    # accent stripe — simulated with a coloured table cell
    accent_row = [["📊  DATA ANALYSIS REPORT"]]
    ...
    meta_rows = [
        ...
        ["Rows",         _fmt(ov.get("rows","—")),   "Columns", _fmt(ov.get("columns","—"))],
        ["Numeric Cols", _fmt(ov.get("numeric_cols","—")), "Categorical", _fmt(ov.get("categorical_cols","—"))],
        ["Missing Cells",f"{_fmt(ov.get('missing_cells','—'))}  ({ov.get('missing_pct',0):.1f}%)",
         "Duplicates",   _fmt(ov.get("duplicate_rows","—"))],
    ]
    meta_table_data = []
...
def _summary_page(story, styles, analysis):
    ...
    # Insights
    if ins:
        story.append(Paragraph("💡 Key Insights", S["sub_heading"]))
...
def _numeric_page(story, styles, analysis, charts):
    ...
    if "boxplot" in charts:
        story.append(Paragraph("Distribution Overview — Box Plot", S["sub_heading"]))
        img = _img_from_bytes(charts["boxplot"], width=W - 2*MARGIN)
        story.append(img)
        story.append(Paragraph("Boxes show IQR (Q1–Q3). Whiskers extend to 1.5×IQR. Circles are outliers.", S["caption"]))
        story.append(Spacer(1, 0.4*cm))
...
def _correlation_page(story, styles, analysis, charts):
    ...
    if pairs:
        ...
        for p in pairs[:8]:
            ...
            rows_data.append([p["col1"], p["col2"], f"{r:.4f}", f"{dir_} · {strength}"])
    ...
    if "corr_heatmap" in charts:
        ...
        story.append(img)
        story.append(Paragraph("Values range from −1 (perfect negative) to +1 (perfect positive).", S["caption"]))
        story.append(Spacer(1, 0.4*cm))
    ...
    if mis:
        ...
        for col, d in sorted(mis.items(), key=lambda x: -x[1]["pct"]):
            ...
            severity = "Critical (>50%)" if pct > 50 else \
                       "High (20–50%)" if pct > 20 else \
                       "Moderate (5–20%)" if pct > 5 else "Low (<5%)"
...
def _appendix_page(story, styles, analysis):
    ...
    story.append(Paragraph("Appendix — Column Inventory", S["section_heading"]))
    story.append(_hr(styles))
...
def build_report(analysis: dict, charts: dict, output_path: str) -> str:
    ...
    doc = SimpleDocTemplate(
        ...
        title=f"Analysis Report — {fname}",
        author="AutoReport · CODTECH Task 2",
    )
